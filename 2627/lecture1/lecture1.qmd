...
subtitle: "Lecture 1 - LAS"
author:   "Don van den Bergh"
date: last-modified
format:
    dtslides-revealjs:
        theme: [tilburg-university.scss]
        logo: logo_tu_small.svg
        slide-number: c/t
        # chalkboard: true
        toc: true
        toc-depth: 1
...
- [Self study questions](https://cjvanlissa.github.io/stats12/introduction.html#formative-test)
- [Weekly quizzes](https://cjvanlissa.github.io/stats12/introduction.html#tutorial)
- [Assignments](https://cjvanlissa.github.io/stats12/statistics1.html#assignments)

## GitBook -- outdated!

All information under [course-schedule](https://cjvanlissa.github.io/stats12/statistics1.html#course-schedule) is outdated.

- Classical lectures
- Dates are all from past year

# Dictionary of Basic Concepts {.section-slide}
...
ii <- c(1:6, 8, 10, 12, 14, 16, 18)
cnms <- colnames(dataset[, ii])
cnms2 <- gsub("_response", "", cnms)
d <- dataset[, ii]
colnames(d) <- cnms2
mydata <- head(d, 30)
# mydata <- head(dataset, 50)
DT::datatable(
  mydata,
  options = list(
  searching = FALSE,
...
::: {.r-stack style="height: 50vh;"}
![](lecture1_images/sample_definition.png){.fragment style="max-width: 70vw; max-height: 50vh; width: auto; height: auto; object-fit: contain;"}

![](lecture1_images/sample_supermarket1.jpg){.fragment style="max-width: 70vw; max-height: 50vh; width: auto; height: auto; object-fit: contain;"}

![](lecture1_images/sample_supermarket2.jpg){.fragment style="max-width: 70vw; max-height: 50vh; width: auto; height: auto; object-fit: contain;"}

![](lecture1_images/sample_paint.jpg){.fragment style="max-width: 70vw; max-height: 50vh; width: auto; height: auto; object-fit: contain;"}
:::
...
A common pattern is _upper case_ or _Greek_ symbols ($\sigma$) for the population and _lower case_ or _Roman_ symbols ($s$) for the sample.
...
# Role of Data in Scientific Research {.section-slide}
...
## A lot of sampling theory is just _cooking_

* Suppose you want to know if your soup needs more salt.
    + You don't eat the whole soup, you _sample_ it.
    + You stir the soup so to take a _random_ sample.
    + If you keep tasting from the spoon while adding salt to the soup, the sample is _non-random_.

# Measurement Level {.section-slide}
...
Ordinal because distances are meaningless.
In practice often analyzed as interval (which is suboptimal).

::: {.notes}

Happiness: 2 - 4 vs. 6 - 8 _is not_ the same, so not interval.
IQ score: 100 vs. 115 _does not_ means not twice as happy, so not ratio.
Hours worked: 40 vs 20 _does_  means twice as much, so ratio.
...
- Milliseconds since the big bang? <!-- Ratio -->
- Unix time (the number of non-leap seconds that have elapsed since 00:00:00 UTC on 1 January 1970) <!-- Ratio or intervals -->
- Intervals: 0 - 5, 6 - 10, 11 - 15 <!-- Intervals -->
- Months <!-- Ordinal -->
...
Units of $S^2$ are squared; if you measured height in meter $m$, $S^2_{height}$ is expressed in $m^2$
...
![](images/Lecture 111.png){fig-align="center"}

But... most Australians are not!

::: footer

Source: <https://www.volkskrant.nl/columns-opinie/australiers-zijn-gemiddeld-miljonair-maar-daar-hebben-de-meeste-australiers-helemaal-niks-aan~b8aa3fd3>

[without paywall](https://archive.ph/wuIQA)
...
::: {layout-ncol=2}
![](lecture1_images/spurious_cor_lamont.svg)

![](lecture1_images/spurious_cor_boss.svg)
:::
...
Source: [left](https://www.tylervigen.com/spurious/correlation/5950_popularity-of-the-first-name-lamont_correlates-with_burglary-rates-in-the-us) [right](https://www.tylervigen.com/spurious/correlation/4994_popularity-of-the-like-a-boss-meme_correlates-with_google-searches-for-how-to-cut-own-hair)
...
- A few univariate and bivariate descriptives statistics



# See you next week! {.section-slide .unlisted}
